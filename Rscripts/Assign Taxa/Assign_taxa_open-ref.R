...
setwd("~/projects/DenoiseCompare_Out/biscuit/med/OpenRef_OTU/final/")
...
which(rownames(biscuit_table) != biscuitIDtemp$V1)

rownames(biscuit_table) <- biscuitIDtemp$V2

which(rownames(biscuit_table) != biscuitIDtemp$V2)
...
setwd("~/projects/DenoiseCompare_Out/Blueberry/med/OpenRef_OTU/final/")
...
#check to make sure row names for biscuit table are the same as the replacement.
#check if any don't equal each other
which(rownames(blueberry_table) != blueberryIDtemp$V1)

rownames(blueberry_table) <- blueberryIDtemp$V2

which(rownames(blueberry_table) != blueberryIDtemp$V2)
...
blueberry_ID["ffe0fd28ad12a5cc90eecbbb9ec243c13db46c8b", "V2"] == blueberryIDtemp["ffe0fd28ad12a5cc90eecbbb9ec243c13db46c8b", "V2"]
...
setwd("~/projects/DenoiseCompare_Out/Exercise_med/med/OpenRef_OTU/final/")
...
#check to make sure row names for biscuit table are the same as the replacement.
#check if any don't equal each other
which(rownames(Exercise_table) != ExerciseIDtemp$V1)

rownames(Exercise_table) <- ExerciseIDtemp$V2

which(rownames(Exercise_table) != ExerciseIDtemp$V2)
