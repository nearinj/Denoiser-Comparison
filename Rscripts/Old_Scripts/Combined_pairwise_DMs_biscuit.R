...
Dada_only <- read.table("Dada/distances/weighted_unifrac_Dada2_samp_fix.txt", sep="\t", header=T, row.names = 1, check.names = F, stringsAsFactors = F)
Unoise_only <- read.table("Unoise/distances/weighted_unifrac_Unoise_samp_fix.txt", sep="\t", header=T, row.names=1, check.names=F, stringsAsFactors = F)
Deblur_only <- read.table("Deblur/distances/weighted_unifrac_Deblur_samp_fix.txt", sep="\t", header=T, row.names = 1, check.names = F, stringsAsFactors = F)
OpenRef_only <- read.table("Open/distances/weighted_unifrac_open-ref-biscuit_samp_fix.txt", sep="\t", header=T, row.names=1, check.names = F, stringsAsFactors = F)
...
DadaVUnoise_format <- format_DM(DadaVUnoise, "Dada_", "Unoise__")
...
DeblurVUnoise_format <- format_DM(DeblurVUnoise, "Deblur_", "Unoise__")
UnoiseVOpenref_format <- format_DM(UnoiseVOpenref, "open-ref_", "Unoise__")

sample_name_list <- gsub("Deblur_", "", colnames(Deblur_only))

#filter tables to only contain the samples of interest.

DadaVUnoise_filt <- DadaVUnoise_format[(match(paste("Dada_",sample_name_list, sep=""), rownames(DadaVUnoise_format))), 
                                       match(paste("Unoise__", sample_name_list, sep=""), colnames(DadaVUnoise_format))]
...
DeblurVUnoise_filt <- DeblurVUnoise_format[match(paste("Deblur_", sample_name_list, sep=""), rownames(DeblurVUnoise_format)),
                                           match(paste("Unoise__", sample_name_list, sep=""), colnames(DeblurVUnoise_format))]
...
UnoiseVOpenref_filt <- UnoiseVOpenref_format[match(paste("open-ref_", sample_name_list, sep=""), rownames(UnoiseVOpenref_format)),
                                             match(paste("Unoise__", sample_name_list, sep=""), colnames(UnoiseVOpenref_format))]
...
Unoise_filt <- Unoise_only[match(paste("Unoise__", sample_name_list, sep=""), rownames(Unoise_only)),
                           match(paste("Unoise__", sample_name_list, sep=""), colnames(Unoise_only))]
dim(Unoise_filt)
...
U_Dada_only <- read.table("Dada/distances/unweighted_unifrac_Dada2_samp_fix.txt", sep="\t", header=T, row.names = 1, check.names = F, stringsAsFactors = F)
U_Unoise_only <- read.table("Unoise/distances/unweighted_unifrac_Unoise_samp_fix.txt", sep="\t", header=T, row.names=1, check.names=F, stringsAsFactors = F)
U_Deblur_only <- read.table("Deblur/distances/unweighted_unifrac_Deblur_samp_fix.txt", sep="\t", header=T, row.names = 1, check.names = F, stringsAsFactors = F)
U_OpenRef_only <- read.table("Open/distances/unweighted_unifrac_open-ref-biscuit_samp_fix.txt", sep="\t", header=T, row.names=1, check.names = F, stringsAsFactors = F)
...
#format the unweighted table

U_DadaVUnoise_format <- format_DM(U_DadaVUnoise, "Dada_", "Unoise__")
...
U_DeblurVUnoise_format <- format_DM(U_DeblurVUnoise, "Deblur_", "Unoise__")
U_UnoiseVOpenref_format <- format_DM(U_UnoiseVOpenref, "open-ref_", "Unoise__")

sample_name_list <- gsub("Deblur_", "", colnames(U_Deblur_only))

#filter the tables

U_DadaVUnoise_filt <- U_DadaVUnoise_format[(match(paste("Dada_",sample_name_list, sep=""), rownames(U_DadaVUnoise_format))), 
                                       match(paste("Unoise__", sample_name_list, sep=""), colnames(U_DadaVUnoise_format))]
dim(U_DadaVUnoise_filt)
U_DadaVOpenref_filt <- U_DadaVOpenref_format[(match(paste("open-ref_", sample_name_list, sep=""), rownames(U_DadaVOpenref_format))),
                                         match(paste("Dada_", sample_name_list, sep=""), colnames(U_DadaVOpenref_format))]
dim(U_DadaVOpenref_filt)
U_DeblurVDada_filt <- U_DeblurVDada_format[(match(paste("Deblur_", sample_name_list, sep=""), rownames(U_DeblurVDada_format))),
                                       match(paste("Dada_", sample_name_list, sep=""), colnames(U_DeblurVDada_format) )]
dim(U_DeblurVDada_filt)
U_DeblurVopenref_filt <- U_DeblurVopenref_format[match(paste("Deblur_", sample_name_list, sep=""), rownames(U_DeblurVopenref_format)), 
                                             match(paste("open-ref_", sample_name_list, sep=""), colnames(U_DeblurVopenref_format))]
dim(U_DeblurVopenref_filt)
U_DeblurVUnoise_filt <- U_DeblurVUnoise_format[match(paste("Deblur_", sample_name_list, sep=""), rownames(U_DeblurVUnoise_format)),
                                           match(paste("Unoise__", sample_name_list, sep=""), colnames(U_DeblurVUnoise_format))]

dim(U_DeblurVUnoise_filt)

U_UnoiseVOpenref_filt <- U_UnoiseVOpenref_format[match(paste("open-ref_", sample_name_list, sep=""), rownames(U_UnoiseVOpenref_format)),
                                             match(paste("Unoise__", sample_name_list, sep=""), colnames(U_UnoiseVOpenref_format))]
dim(U_UnoiseVOpenref_filt)

U_Dada_filt <- U_Dada_only[match(paste("Dada_", sample_name_list, sep=""), rownames(U_Dada_only)),
                       match(paste("Dada_", sample_name_list, sep=""), colnames(U_Dada_only))]
dim(U_Dada_filt)

U_Unoise_filt <- U_Unoise_only[match(paste("Unoise__", sample_name_list, sep=""), rownames(U_Unoise_only)),
                           match(paste("Unoise__", sample_name_list, sep=""), colnames(U_Unoise_only))]
dim(U_Unoise_filt)

U_Deblur_filt <- U_Deblur_only[match(paste("Deblur_", sample_name_list, sep=""), rownames(U_Deblur_only)),
                           match(paste("Deblur_", sample_name_list, sep=""), colnames(U_Deblur_only))]
dim(U_Deblur_filt)

U_Open_filt <- U_OpenRef_only[match(paste("open-ref_", sample_name_list, sep=""), rownames(U_OpenRef_only)),
                          match(paste("open-ref_", sample_name_list, sep=""), colnames(U_OpenRef_only))]
dim(U_Open_filt)
