...
setwd("~/projects/DenoiseCompare_Out/biscuit/med/COMBINED/biom/")
...
#some samples will be lost due to rarefaction of 4259

Dada2_table <- Dada2_table[, colnames(Deblur_table)]
Unoise_table <- Unoise_table[, colnames(Deblur_table)]

colnames(Dada2_table) == colnames(Deblur_table)
colnames(Dada2_table) == colnames(Unoise_table)
colnames(Unoise_table) == colnames(Deblur_table)
...
colnames(Combined) == colnames(Dada2_table)
colnames(Combined) == colnames(Deblur_table)
...
#write TSV file
write.table(Combined, file="Combined_biscuit.tsv", sep="\t", quote=F, col.names = NA)
...
#write to a tsv file
write.table(taxa, file="taxa_Combined_biscuit.tsv", sep="\t", quote=F, col.names = NA)
