...
setwd("/home/jacob/projects/DenoiseCompare_Out/biscuit/med/COMBINED/biom/fixed_combined/distances/")
...
proportions <- read.table("weighted_unifrac_PC_cord.txt", sep = "\t", nrow=1, skip=4)

#read in the number oof samples
SampleNum <- read.table("weighted_unifrac_PC_cord.txt", sep = "\t", nrow=1)

#Get coordinates for samples.
sample_cord <- read.table("weighted_unifrac_PC_cord.txt", 
                          sep ="\t", skip = 9, 
                          nrow = SampleNum[[2]],
...
unweight_porp <- read.table("unweighted_unifrac_PC_cord.txt", sep="\t", skip=4, nrow=1)
unweight_samples <- read.table("unweighted_unifrac_PC_cord.txt", sep="\t", nrow=1)
unweight_coords <- read.table("unweighted_unifrac_PC_cord.txt", 
                              sep ="\t", skip = 9, 
...
unweighted_rm_noPos <- unweighted_filtered[-which(unweighted_filtered$Pipeline == "Deblur-noPos"),]

noPos_unweighted_plot <- ggplot(data=unweighted_rm_noPos, aes(PC1, PC2)) +
  geom_point(aes(fill=sample, size=1.5, shape=Pipeline)) +
  theme_minimal() +
  xlab(paste("PC1 (",round(unweight_porp$V1, 2)*100,"%)", sep="")) +
  ylab(paste("PC2 (",round(unweight_porp$V2, 2)*100,"%)", sep="")) +
...
for (sample in levels(unweighted_rm_noPos$sample)) {
  noPos_unweighted_plot <- noPos_unweighted_plot + geom_line(data = unweighted_rm_noPos[which(unweighted_rm_noPos$sample == sample),], aes(x = PC1, y = PC2))
}

plot(noPos_unweighted_plot)


#plot final figure

#need plots generated from Bray_curtis_biscuit_updated and Weighted_Unifrac_biscuit
library(cowplot)

biscuit_bray_weight_plot <- plot_grid(Unifrac_box, recorded_Bray_Curtis_box, noPos_unifrac_plot, bray_curtis_plot, labels="AUTO", rel_heights = c(1,2))
biscuit_bray_weight_plot
#9 by 19.5
