...
setwd("/home/jacob/projects/DenoiseCompare_Out/Exercise_med/med/COMBINED/biom/final_combined/sequences/pynast_aligned_seqs/sep-align/")
...
proportions <- read.table("weighted_PC_coords.txt", sep = "\t", nrow=1, skip=4)

#read in the number oof samples
SampleNum <- read.table("weighted_PC_coords.txt", sep = "\t", nrow=1)

#Get coordinates for samples.
sample_cord <- read.table("weighted_PC_coords.txt", 
                          sep ="\t", skip = 9, 
                          nrow = SampleNum[[2]],
...
unifrac_plot <- ggplot(data=unifrac_combined_Exercise, aes(PC1, PC2)) +
...
for (sample in levels(unifrac_combined_Exercise$sample)) {
  unifrac_plot <- unifrac_plot + geom_line(data = unifrac_combined_Exercise[which(unifrac_combined_Exercise$sample == sample),], aes(x = PC1, y = PC2))
}

plot(unifrac_plot)

### orginal weighted unifrac

unifrac_combined_Exercise_orig <- unifrac_combined_Exercise[-(which(unifrac_combined_Exercise$Pipeline=="open-ref")),]

unifrac_plot_orig <- ggplot(data=unifrac_combined_Exercise_orig, aes(PC1, PC2)) +
  geom_point(aes(fill=sample, size=1.5, shape=Pipeline)) +
  theme_minimal() +
  xlab(paste("PC1 (",round(proportions$V1, 2)*100,"%)", sep="")) +
  ylab(paste("PC2 (",round(proportions$V2, 2)*100,"%)", sep="")) +
...
for (sample in levels(unifrac_combined_Exercise_orig$sample)) {
  unifrac_plot_orig <- unifrac_plot_orig + geom_line(data = unifrac_combined_Exercise_orig[which(unifrac_combined_Exercise_orig$sample == sample),], aes(x = PC1, y = PC2))
}
...
U_unifrac_plot <- ggplot(data=U_unifrac_combined_Exercise, aes(PC1, PC2)) +
  geom_point(aes(fill=sample, size=1.5, shape=Pipeline)) +
  theme_minimal() +
...
for (sample in levels(U_unifrac_combined_Exercise$sample)) {
  U_unifrac_plot <- U_unifrac_plot + geom_line(data = U_unifrac_combined_Exercise[which(U_unifrac_combined_Exercise$sample == sample),], aes(x = PC1, y = PC2))
}
...
bray_curtis_dm <- read.table("../../../bray_curtis/distances/bray_curtis_rare_CombinedV2_Exercise_taxa_L6.txt",
                             sep="\t", 
                             header=T, 
...
bray_curtis_dm_NO_pos <- bray_curtis_dm[-(grep("Deblur-noPos", rownames(bray_curtis_dm))),]
bray_curtis_dm_NO_pos <- bray_curtis_dm_NO_pos[,-grep("Deblur.noPos", colnames(bray_curtis_dm_NO_pos))]


bray_curtis_NMDS <- metaMDS(as.dist(bray_curtis_dm_NO_pos), trymax=1000)
...
samples_to_keep <- bray_curtis_NMDS_df[which(bray_curtis_NMDS_df$Pipeline == "Deblur"), "sample"]

bray_curtis_NMDS_df_filt <- bray_curtis_NMDS_df[bray_curtis_NMDS_df$sample %in% as.character(samples_to_keep),]

bray_curtis_plot <- ggplot(data=bray_curtis_NMDS_df_filt, aes(NMDS1, NMDS2)) +
  geom_point(aes(fill=sample, size=1.5, shape=Pipeline)) +
  theme_minimal() +
  xlab("NMDS1") +
  ylab("NMDS2") +
...
for (sample in levels(bray_curtis_NMDS_df_filt$sample)) {
  bray_curtis_plot <- bray_curtis_plot + geom_line(data = bray_curtis_NMDS_df_filt[which(bray_curtis_NMDS_df_filt$sample == sample),], aes(x = NMDS1, y = NMDS2))
}
...
#read in proportions for the PC
unweighted_prop <- read.table("unweighted_PC_cords.txt", sep = "\t", nrow=1, skip=4)

#read in the number oof samples
unweighted_SampleNum <- read.table("unweighted_PC_cords.txt", sep = "\t", nrow=1)

#Get coordinates for samples.
unweighted_sample_cord <- read.table("unweighted_PC_cords.txt", 
                          sep ="\t", skip = 9, 
                          nrow = unweighted_SampleNum[[2]],
                          header=FALSE, row.names=1)
