...
DeblurVDeblur.noPos <- as.matrix(DM[grep("Deblur_", rownames(DM)), grep("Deblur-noPos_",colnames(DM))])
DeblurVDeblur.noPos_order <- reorder_pipeline_samples(DeblurVDeblur.noPos, row_prefix = "Deblur_", col_prefix = "Deblur-noPos_")
...
noPos_Bray_curtis <- recordPlot(noPOs_boxplot)

### final figure then would be all three box plots in three rows... 1 column!
