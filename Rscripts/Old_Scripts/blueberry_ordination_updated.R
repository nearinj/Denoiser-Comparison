...
setwd("/home/jacob/projects/DenoiseCompare_Out/Blueberry/med/COMBINED/biom/fixed_combined/distances/")
...
proportions <- read.table("weighted_unifrac_PC_cords.txt", sep = "\t", nrow=1, skip=4)

#read in the number oof samples
SampleNum <- read.table("weighted_unifrac_PC_cords.txt", sep = "\t", nrow=1)

#Get coordinates for samples.
sample_cord <- read.table("weighted_unifrac_PC_cords.txt", 
                          sep ="\t", skip = 9, 
                          nrow = SampleNum[[2]],
...
for (sample in levels(unifrac_combined_blueberry_temp$sample)) {
  unifrac_plot <- unifrac_plot + geom_line(data = unifrac_combined_blueberry_temp[which(unifrac_combined_blueberry_temp$sample == sample),], aes(x = PC1, y = PC2))
}
