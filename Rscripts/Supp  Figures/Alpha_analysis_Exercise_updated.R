...
by_sample <- data.frame(Samples=character(94),
                        ...
                        Open_Shannon=double(94),
                        DADA2_Observed_ASV=integer(94),
                        Deblur_Observed_ASV=integer(94),
                        UNOISE3_Observed_ASV=integer(94),
                        DeblurnoPos_Observed_ASV=integer(94),
                        Open_Observed_OTU=integer(94))
...
by_sample[by_sample$Samples=="FEzW2zC1", "DADA2_Observed_ASV"]
...
by_sample[by_sample$Samples=="FEzW6mzF4", "Deblur_Observed_ASV"]
...
by_sample[by_sample$Samples=="FEzW4zC4", 'UNOISE3_Observed_ASV']


alpha_data["open-ref_FEzW4zF9", "shannon"]
by_sample[by_sample$Samples=="FEzW4zF9", "Open_Shannon"]

alpha_data["open-ref_FEzW6mzC9", "observed_otus"]
by_sample[by_sample$Samples=="FEzW6mzC9", "Open_Observed_OTU"]
###################check all them
...
for(i in samples_to_compare){
  ...
  if(!identical(alpha_data[paste("Unoise__",i,sep=""),"observed_otus"], by_sample[by_sample$Samples==i,"UNOISE3_Observed_ASV"])){
    message("error")
    break
  }else
    j <- j +1
...
DADAvDeblur_OTU <- ggplot(by_sample, aes(x=DADA2_Observed_ASV, y=Deblur_Observed_ASV)) + geom_point() + theme_gdocs() +xlim(50,900) + ylim(50,900) +
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$DADA2_Observed_ASV, by_sample$Deblur_Observed_ASV), colour="black", size = 5, parse=TRUE)
DADAvDeblur_OTU

DadavUnoise_OTU <- ggplot(by_sample, aes(x=DADA2_Observed_ASV, y=UNOISE3_Observed_ASV)) + geom_point() + theme_gdocs() +xlim(50,900) + ylim(50,900) + 
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$DADA2_Observed_ASV, by_sample$UNOISE3_Observed_ASV), colour="black", size = 5, parse=TRUE)
DadavUnoise_OTU

UnoisevDeblur_OTU <- ggplot(by_sample, aes(x=UNOISE3_Observed_ASV, y=Deblur_Observed_ASV)) + geom_point() + theme_gdocs() +xlim(50,900) + ylim(50,900) +
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$UNOISE3_Observed_ASV, by_sample$Deblur_Observed_ASV), colour="black", size = 5, parse=TRUE)
UnoisevDeblur_OTU

DeblurVOpen_shannon <- ggplot(by_sample, aes(x=Deblur_Shannon, y=Open_Shannon)) +geom_point() + theme_gdocs()  +xlim(3,9) +ylim(3,9) + 
  annotate("text", x = 5.5, y = 8.5, label = lm_eqn(by_sample, by_sample$Deblur_Shannon, by_sample$Open_Shannon), colour="black", size = 5, parse=TRUE)
DeblurVOpen_shannon

UnoiseVOpen_shannon <- ggplot(by_sample, aes(x=UNOISE3_Shannon, y=Open_Shannon)) + geom_point() + theme_gdocs()+xlim(3,9) +ylim(3,9) +
  annotate("text", x = 5.5, y = 8.5, label = lm_eqn(by_sample, by_sample$UNOISE3_Shannon, by_sample$Open_Shannon), colour="black", size = 5, parse=TRUE)
UnoiseVOpen_shannon

DadaVOpen_shannon <- ggplot(by_sample, aes(x=DADA2_Shannon, y=Open_Shannon)) + geom_point() + theme_gdocs() +xlim(3,9) +ylim(3,9) +
  annotate("text", x = 5.5, y = 8.5, label = lm_eqn(by_sample, by_sample$DADA2_Shannon, by_sample$Open_Shannon), colour="black", size = 5, parse=TRUE)
DadaVOpen_shannon

DeblurVOpen_OTU <- ggplot(by_sample, aes(x=Deblur_Observed_ASV, y=Open_Observed_OTU)) + geom_point() + theme_gdocs()  +xlim(50,900) + ylim(50,900)+
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$Deblur_Observed_ASV, by_sample$Open_Observed_OTU), colour="black", size = 5, parse=TRUE)
DeblurVOpen_OTU

UnoiseVOpen_OTU <- ggplot(by_sample, aes(x=UNOISE3_Observed_ASV, y=Open_Observed_OTU)) + geom_point() + theme_gdocs()  +xlim(50,900) + ylim(50,900) + 
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$UNOISE3_Observed_ASV, by_sample$Open_Observed_OTU), colour="black", size = 5, parse=TRUE)
UnoiseVOpen_OTU

DadaVOpen_OTU <- ggplot(by_sample, aes(x=DADA2_Observed_ASV, y=Open_Observed_OTU)) + geom_point() + theme_gdocs() +xlim(50,900) + ylim(50,900)+
  annotate("text", x = 325, y = 875, label = lm_eqn(by_sample, by_sample$DADA2_Observed_ASV, by_sample$Open_Observed_OTU), colour="black", size = 5, parse=TRUE)
DadaVOpen_OTU
